...
var jsonTranslate = {
    ...
    discount: ["khuyến mãi", "giảm giá", "tặng quà", "ưu đãi"],
    warranty: ["bảo hành"],
...
function AnswerGameCondition(jsonPhone, phone_property_value) {
    ...
    if (jsonPhone["operating_system"].toLowerCase().search("android") != -1) {
        if (
            jsonPhone.ram >= "4 GB" &&
            jsonPhone.price > 5000000 &&
            jsonPhone.price <= 8000000
        ) {
            result["message"] =
                "Chào anh/chị!Dạ theo em kiểm tra thì Điện thoại " +
                jsonPhone.name +
                " sử dụng chip " +
                jsonPhone.chipset +
                ", cpu: " +
                ...
                ", ram: " +
                jsonPhone.ram +
                " nên đáp ứng tốt nhu cầu chơi game ở mức cấu hình mượt đó ạ. Thông tin đến anh/chị!";
            ...
            result["message"] =
                ...
                ", ram: " +
                jsonPhone.ram +
                " nên đáp ứng nhu cầu chơi game ở mức cấu hình cao đó ạ. Thông tin đến anh/chị!";
            ...
            result["message"] =
                ...
                ", ram: " +
                jsonPhone.ram +
                " nên đáp ứng nhu cầu chơi game ở mức bình thường đó ạ. Thông tin đến anh/chị!";
        }
...
function calBatteryChargingTime(jsonPhone) {
    ...
    if (
        SearchPropertyValue(jsonPhone, "sạc pin nhanh") ||
        SearchPropertyValue(jsonPhone, "sạc nhanh")
    ) {
...
router.post("/queryListPhone", (req, res, next) => {
    var PhoneName = req.body.PhoneName;
    if (PhoneName.search(" ") !=-1){
        PhoneName = PhoneName.substring(0, PhoneName.search(" "));
    }
    ...
    Product.find({
        ...
        function (err, result) {
            if (err) res.send(err);
                ...
                arrayPhoneName = result.map( item => {
                    return item.title;
                });
                res.send(JSON.stringify(arrayPhoneName));
        }
    );
    ...
    async function findResult() {
        try {
            ...
            const response = await axios.post(
                "http://localhost:3030/api/answer/querymongo",
                params
            );
            if (response.data) {
                jsonPhone = response.data;
            } else {
                // call api query listPhoneName
                const listPhoneName = await axios.post(
                    "http://localhost:3030/api/answer/queryListPhone",
                    params
                );
                result["message"] =
                    "Chào anh/chị. Em vừa kiểm tra thì hệ thống bên em không có thông tin của mẫu điện thoại " +
                    phone_name +
                    " . Anh/chị giúp em kiểm tra lại tên của điện thoại với ạ. Thông tin đến anh/chị." + '\n'+ "Danh sách các mẫu điện thoại có cùng hãng mà anh/chị đang tìm kiếm là:" +listPhoneName.data;
                result["isFlag"] = true;
                return result;
            }
            ...
            if (phone_property && !phone_property_value) {
                var translatePhoneProperty = translateProperty(
                    phone_property,
                    jsonTranslate
                );

                var __searchPropertyKey = SearchPropertyKey(
                    jsonPhone,
                    translatePhoneProperty
                );
                if (__searchPropertyKey) {
                    result["message"] =
                        "Chào anh/chị. Sau quá trình em kiểm tra thì máy " +
                        phone_name +
                        " có " +
                        phone_property +
                        ": " +
                        jsonPhone[translatePhoneProperty] +
                        " . Xin thông tin đến anh/chị!";
                    result["isFlag"] = true;
                    return result;
                } else {
                    // tìm giá trị thuộc tính
                    var __searchPropertyValue = SearchPropertyValue(
                        jsonPhone,
                        translatePhoneProperty
                    );
                    ...
                    if (__searchPropertyValue) {
                        ...
                        if (translatePhoneProperty == "isAvailable") {
                            result["message"] =
                                "Chào anh/chị. Hiện tại sản phẩm Điện thoại " +
                                phone_name +
                                " bên em còn hàng đó ạ. Giá của máy là " +
                                jsonPhone.price.toLocaleString("it-IT", {
                                    style: "currency",
                                    currency: "VND"
                                }) +
                                " . Anh/chị có thể tham khảo nha. Thông tin đến anh/chị";
                            result["isFlag"] = true;
                        } else if (translatePhoneProperty == "checkNew") {
                            result["message"] =
                                "Chào anh/chị. Hiện tại bên em chỉ kinh doanh điện thoại mới nguyên seal, chính hãng Việt Nam ạ. Bên em không có kinh doanh máy cũ. Thông tin đến anh/chị";
                            result["isFlag"] = true;
                        } else if (translatePhoneProperty == "discount") {
                            result["message"] =
                                "Chào anh/chị. Dạ hiện sản phẩm Điện thoại " +
                                phone_name +
                                " bên em đang có chương trình khuyến mãi. Giá hiện tại là " +
                                jsonPhone.price.toLocaleString("it-IT", {
                                    style: "currency",
                                    currency: "VND"
                                }) +
                                ". Giảm thêm 5% (" +
                                ((jsonPhone.price * 5) / 100).toLocaleString("it-IT", {
                                    style: "currency",
                                    currency: "VND"
                                }) +
                                ") cho khách hàng mua online có sinh nhật trong tháng này ạ. Xin thông tin đến anh/chị";
                            result["isFlag"] = true;
                        } else if (translatePhoneProperty == "warranty") {
                            result["message"] =
                                "Chào anh/chị. Dạ hiện sản phẩm " +
                                phone_name +
                                " hỗ trợ Bảo hành chính hãng 12 tháng và 1 đổi 1 trong vòng 1 tháng nếu sản phẩm có lỗi từ nhà sản xuất. Thông tin đến anh/chị";
                            result["isFlag"] = true;
                        } else if (translatePhoneProperty == "paperInstallment") {
                            result["message"] =
                                "Chào anh/chị ! Dạ nếu anh/chị đủ 20 - 60 tuổi, có CMND và Hộ khẩu hoặc CMND và Bằng lái xe thì anh/chị có thể mua trả góp được rồi anh/chị nha, nếu anh/chị cần bên em hỗ trợ gì thêm anh/chị có thể phản hồi bên dưới anh/chị nhé. Thông tin đến anh/chị !";
                            result["isFlag"] = true;
                        } else if (translatePhoneProperty.search("game") != -1) {
                            result["message"] = AnswerGameCondition(
                                jsonPhone,
                            ...
                            result["message"] =
                                "Chào anh/chị. Sau quá trình kiểm tra dữ liệu của hệ thống bên em thì máy " +
                                phone_name +
                                " không có hỗ trợ " +
                                phone_property;
                            result["isFlag"] = false;
                ...
                var translatePhonePropertyValue = translateProperty(
                    phone_property_value,
                    jsonTranslate
                );
                // kiểm tra dữ liệu
                if (jsonPhone[translatePhoneProperty]) {
                    var __findValueInKey = jsonPhone[translatePhoneProperty]
                        .toLowerCase()
                        .includes(translatePhonePropertyValue.toString().toLowerCase());
                } else if (
                    translatePhoneProperty.search("game") != -1 &&
                    translatePhonePropertyValue.search("configure") != -1
                ) {
                    result["message"] = AnswerGameCondition(
                        jsonPhone,
                        phone_property_value
                    ...
                    result["message"] =
                        "Chào anh/chị. Sau quá trình kiểm tra dữ liệu của hệ thống bên em thì bên em không đủ yếu tố để trả lời câu hỏi của anh/chị. Mong anh/chị thông cảm giúp bên em. Em vừa tham khảo google thì có thông tin như sao";
                    result["isFlag"] = false;
                }
                ...
                if (__findValueInKey) {
                    result["message"] =
                        "Chào anh/chị. Sau quá trình em kiểm tra thì thấy máy " +
                        phone_name +
                        " có " +
                        phone_property +
                        " hỗ trợ " +
                    ...
                    result["message"] =
                        "Chào anh/chị. Sau quá trình em kiểm tra thì thấy " +
                        phone_property +
                        " chỉ có " +
                        jsonPhone[translatePhoneProperty] +
                        " và không hỗ trợ " +
...
router.post("/yesno", (req, res, next) => {
    ...
    ActionYesNo(PhoneName, PhoneProperty, PhonePropertyValue).then(function (
        result
    ) {
        resultTemp = result;
        res.send(resultTemp);
            ...
            if (phone_property && !phone_property_value) {
                ...
                if (__searchPropertyKey) {
                    ...
                    if (__searchPropertyValue) {
                        ...
                        if (translatePhoneProperty == "isAvailable") {
                            ...
                            result["message"] =
                                "Chào anh/chị ! Dạ sản phẩm " +
                                phone_name +
                                " là máy của hãng " +
                                getCompanyName(phone_name) +
                                " . Thông tin đến anh/chị !";
                            result["isFlag"] = true;
                        } else if (translatePhoneProperty.search("configure") != -1) {
                            result["message"] =
                                "Chào anh/chị ! Dạ em kiểm tra thì thấy sản phẩm " +
                                phone_name +
                                " có cấu hình là Chip " +
                                jsonPhone.chipset +
                                " , CPU: " +
                            ...
                            result["message"] =
                                "Chào anh/chị. Sau quá trình kiểm tra dữ liệu của hệ thống bên em thì không đủ thông tin để trả lời cho câu hỏi này.Em vừa tham khảo qua google thì có thông tin như sau.";
                            result["isFlag"] = false;
                        }
                        return result;
                    }
                ...
                if (translatePhonePropertyValue == "BatteryCalculator") {
                    if (translatePhoneProperty == "battery_technology") {
                        ...
                        result["message"] =
                            "Chào anh/chị. Dạ theo em kiểm tra sản phẩm " +
                            phone_name +
                            " có thể sạc đầy pin trong khoảng " +
                            chargeTime +
                            " anh/chị nhé. Thông tin đến anh/chị";
                        result["isFlag"] = true;
                    } else {
                        ...
                        result["message"] =
                            "Chào anh/chị. Dạ sản phẩm " +
                            phone_name +
                            " có thể sử dụng trong khoảng " +
                            resultBatteryUsageTime +
                            " tuỳ độ sáng màn hình, kết nối, tác vụ,... anh/chị nhé. Thông tin đến anh/chị.";
                        result["isFlag"] = true;
                    }
...
router.post("/what", (req, res, next) => {
    ...
    ActionWhat(PhoneName, PhoneProperty, PhonePropertyValue).then(function (
        result
    ) {
        resultTemp = result;
        res.send(resultTemp);
...
async function ActionCompare(PhoneNameFirst, PhoneNameSecond, PhoneProperty) {
    // init result
    var result = {
        message: "",
        isFlag: false
    ...
    async function findResult() {
        try {
            // query by phoneName
            paramsFirst = {
                PhoneName: PhoneNameFirst,
                PhoneProperty: "price",
                MongoProperty: "price"
            };

            paramsSecond = {
                PhoneName: PhoneNameSecond,
                PhoneProperty: "price",
                MongoProperty: "price"
            };
            ...
            if (!responseFirst.data) {
                ...
                jsonPhoneFirst = responseFirst.data;
                const responseSecond = await axios.post(
                    "http://localhost:3030/api/answer/querymongo",
                    paramsSecond
                );
                jsonPhoneSecond = responseSecond.data;
                if (!responseSecond.data) {
                    ...
                    var __searchPropertyKeyFirst = SearchPropertyKey(
                        jsonPhoneFirst,
                        translatePhoneProperty
                    );
                    var __searchPropertyKeySecond = SearchPropertyKey(
                        jsonPhoneSecond,
                        translatePhoneProperty
                    );
                    console.log(translatePhoneProperty);
                    console.log(__searchPropertyKeyFirst);
                    console.log(__searchPropertyKeySecond);
                    if (__searchPropertyKeyFirst && __searchPropertyKeySecond) {
                        result["message"] =
                            "Chào anh/chị. Sau quá trình em kiểm tra thì máy " +
                            PhoneNameFirst +
                            " có " +
                            PhoneProperty +
                            ": " +
                            jsonPhoneFirst[translatePhoneProperty] +
                            " . Và máy " +
                            PhoneNameSecond +
                            " có " +
                            PhoneProperty +
                            ": " +
                            jsonPhoneSecond[translatePhoneProperty] +
                            ". Xin thông tin đến anh/chị!";
                        result["isFlag"] = true;
                        return result;
                    } else if (translatePhoneProperty.search("game") != -1) {
                        message =
                            AnswerGameCondition(jsonPhoneFirst, "").message +
                            "\n" +
                            AnswerGameCondition(jsonPhoneSecond, "").message;
                        result["message"] = message;
                        result["isFlag"] = true;
                        return result;
                    } else if (!__searchPropertyKeyFirst || __searchPropertyKeySecond) {
                        linkCompare = `http://localhost:4200/phone-compare?name1=${PhoneNameFirst}&name2=${PhoneNameSecond}`;
                        var message =
                            "Chào anh/chị. Sau quá trình em kiểm tra thì thấy dữ liệu cần so sánh hơi nhiều. Vì thế, em lập ra 1 bảng so sánh chi tiết của điện thoại " +
                            PhoneNameFirst +
                            " và điện thoại " +
                            PhoneNameSecond +
                            ". Em gửi anh/chị đường link tham khảo: " +
                            url.format(linkCompare);
                        result["message"] = message;
                        return result;
                    }
                }
...
router.post("/compare", (req, res, next) => {
    ...
    if (PhoneNameFirst=='')
    {
        ActionWhat(PhoneNameFirst, PhoneProperty, '').then(function (
            result
        ) {
            resultTemp = result;
            res.send(resultTemp);
    ...
    else if (PhoneNameSecond=='')
    {
        ActionWhat(PhoneNameSecond, PhoneProperty, '').then(function (
            result
        ) {
            resultTemp = result;
            res.send(resultTemp);
    ...
    // if (PhoneNameFirst && PhoneNameSecond && !PhoneProperty) {
    //     linkCompare = `http://localhost:4200/phone-compare?name1=${PhoneNameFirst}&name2=${PhoneNameSecond}`;
    ...
    if (PhoneNameFirst && PhoneNameSecond && PhoneProperty) {
        ActionCompare(PhoneNameFirst, PhoneNameSecond, PhoneProperty).then(function (
            result
        ) {
            resultTemp = result;
    ...
    } else if (PhoneNameFirst && PhoneNameSecond) {
        linkCompare = `http://localhost:4200/phone-compare?name1=${PhoneNameFirst}&name2=${PhoneNameSecond}`;
        var message =
            "Chào anh/chị. Sau quá trình em kiểm tra thì thấy dữ liệu cần so sánh hơi nhiều. Vì thế, em lập ra 1 bảng so sánh chi tiết của điện thoại " +
            PhoneNameFirst +
